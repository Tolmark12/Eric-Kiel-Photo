...
function ajaxClick(a){a.match(/\?/g)!=null?jsonLink=a.replace(/\?/g,".json?"):jsonLink=a+".json";if(!moveToPane(a))$('.bread-crumb a[rel="address:'+a+'"] > span').length>0?window.location=a:(history.pushState({},"",a),popstateLoaded||(popstateLoaded=!0,$(window).bind("popstate",function(){if(!moveToPane(location.pathname))window.location=location})),$.get(jsonLink,function(b){$(".buttons").hide();$(".buttons").append("<div data-path='"+a+'\'><input type="submit" class="button-thin cancel back" value="Back">'+
(b.toolbar!=null?b.toolbar:"")+"</div>");$(".buttons > div:not(div:last)").hide();var c=b.content,d=$(".view-port-container > div:last");$(".view-port-container:first").width();$(".view-port-container > div");d.after($(c).attr("data-path",a));$(".view-port-container > div").width(1005);$(".view-port-container > div:last").css("float","left");$(".buttons > div:last > .back").unbind("click");$(".buttons > div:last > .back").click(function(a){history.back();a.preventDefault();return!1});$(".bread-crumb ul").hide();
$(".bread-crumb ul:last").after($(b.breadcrumb).attr("data-path",a));moveToPane(a);eval(b.javascript)},"json"));return!1}
